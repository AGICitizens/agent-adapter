...
## ADR-015: Single Deployment-Wide Encryption Key for Secrets
- **Date**: 2026-04-07
- **Status**: Accepted
- **Context**: The secrets store (Task 1.3) needs an encryption key for AES-256-GCM. Two options: a single deployment-wide key from an env var, or envelope encryption with per-provider data encryption keys (DEKs) wrapped by a master key (KEK).
- **Decision**: Use a single 256-bit key provided via environment variable (`SECRETS_ENCRYPTION_KEY`, 64 hex chars). The key is referenced in the YAML config as `encryptionKey: ${SECRETS_ENCRYPTION_KEY}` and validated at startup. All providers in a deployment share the same key.
- **Consequences**: Simple setup for self-hosted (single provider) deployments — one env var, no key management infrastructure. Key rotation requires re-encrypting all secrets (acceptable at current scale). Hosted/multi-tenant mode (Phase 6) will need envelope encryption so that each provider's secrets are isolated cryptographically; this is deferred intentionally.
- **Alternatives considered**: Envelope encryption now (rejected — premature complexity for single-tenant self-hosted mode, no multi-tenant requirement until Phase 6), Node.js built-in crypto (rejected in favor of @noble/ciphers — audited, pure JS, no native bindings), per-provider keys from day one (rejected — self-hosted mode has exactly one provider, so the abstraction adds cost with no benefit).
